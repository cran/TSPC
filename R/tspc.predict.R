...
tspc.predict <- function (object, data, newdata, threshold, n.components = 3, prediction.type = c("continuous", "discrete", "nonzero"), n.class = 2) 
{
    ...
    if (prediction.type == "continuous" | prediction.type == 
        ...
        xtemp = t(scale(t(xtemp), center = x.sml.svd$feature.means, 
						scale = FALSE))
        scal = apply(scale(abs(x.sml.svd$u), center = FALSE, scale = x.sml.svd$d), 
					 2, sum)
    ...
    if (prediction.type == "continuous") {
        ...
        for (j in 1:ncol(out)) {
			...
			br = quantile(out0[, j], (0:n.class)/n.class) 
			
			out[,j] = ifelse(out[,j] <= br[2], 1, 2) 
			
        }
        br = quantile(v.pred0.1df, (0:n.class)/n.class)
        v.pred.1df = ifelse(v.pred.1df <= br[2], 1, 2)  
    }
    if (is.matrix(out)) {
